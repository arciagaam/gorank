...
export default function ViewGame() {
    ...
    return (
        <div className="flex flex-col w-full gap-6 md:gap-10 py-6 md:py-10 px-4 md:px-36 pb-72 lg:pb-10">
            <div className="flex flex-col">
                <h1 className='text-lg md:text-xl font-semibold'>{game.name}</h1>
                <p className='text-xs md:text-sm text-gray-500'>{game.facility?.location} | {game.startDate?.toLocaleDateString('en-US', { month: 'long', day: 'numeric' })} | {game.startTime}PM - {game.endTime}PM</p>
            </div>

            {/* Desktop Gallery */}
            <div className="hidden md:grid w-full grid-cols-3 gap-5">
                <div className="col-span-2">
                    <img src={game.facility?.pictures[0]} alt={game.name} className='w-full h-full object-cover rounded-2xl shadow' />
                </div>

                <div className="flex flex-col">
                    <div className="grid grid-rows-3 h-full aspect-video gap-5">
                        {game.facility?.pictures.slice(1, 4).map((picture, index) => (
                            <img
                                key={index}
                                src={picture}
                                alt={`${game.facility?.name} view ${index + 2}`}
            ...
            <div className="md:hidden">
                <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide px-1">
                    {game.facility?.pictures.map((picture, index) => (
                        <img
                            key={index}
                            src={picture}
                            alt={`${game.facility?.name} view ${index + 1}`}
            ...
            <div className="flex flex-col lg:flex-row w-full gap-6 lg:gap-10">
                ...
                <div className="flex flex-col flex-1 gap-6 md:gap-10">
                    ...
                    <div className="flex items-center gap-2">
                        <User size={32} className="md:size-10" />

                        <div className="flex flex-col">
                            <h3 className="text-sm md:text-base">Kubi Brayan</h3>
                            <p className='text-xs md:text-sm text-gray-500'>Organizer</p>
                        </div>
                    </div>

                    <div className="flex flex-col gap-2">
                        <h2 className='text-base md:text-lg font-semibold'>Reminders</h2>
                        <p className="text-sm md:text-base">{game.reminders}</p>
                    </div>

                    <div className="flex flex-col gap-3 md:gap-2">
                        <h2 className='text-base md:text-lg font-semibold'>What this court offers</h2>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-5">
                            {game.facility?.amenities.slice(0, showAllAmenities ? undefined : 4).map((amenity) => (
                            ...
                            {game.facility?.amenities && game.facility.amenities.length > 4 && (
                                <div className="col-span-1 sm:col-span-2 mt-2">
                                    <button
                                        onClick={() => setShowAllAmenities(!showAllAmenities)}
                                        className="text-xs md:text-sm text-gray-600 hover:text-gray-800 underline py-2"
                                    >
                                        {showAllAmenities ? 'Show less' : `Show all ${game.facility.amenities.length} amenities`}
                                    </button>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="flex flex-col gap-2">
                        <h2 className='text-base md:text-lg font-semibold'>Players</h2>
                    </div>
                ...
                <JoinCard game={game as Game} facility={game.facility} />
...
const JoinCard = ({ game, facility }: { game: Game, facility: Facility | undefined }) => {
    ...
    const handleJoin = () => {
        if (!user) {
            Cookies.set('redirect', `/play/${game.id}`);
            navigate('/lets-game');
        } else {
            setJoined(true);
        }
    ...
    return (
        <div className="flex flex-col fixed bottom-0 left-0 right-0 lg:sticky lg:top-10 lg:bottom-auto lg:left-auto lg:right-auto bg-gray-100 rounded-t-2xl lg:rounded-2xl p-4 w-full lg:w-[30%] h-[35vh] lg:h-fit border border-gray-200 shadow gap-2 z-[60]">
            <h2 className='text-xl lg:text-4xl font-semibold'>P200 <span className='text-sm lg:text-base text-gray-500'>per head</span></h2>
            <p className='text-xs lg:text-sm text-gray-500'>Pay at venue</p>

            <ThemedButton onClick={handleJoin} className={twMerge('mt-3 lg:mt-6 py-3 lg:py-2', joined && user && 'hover:brightness-100 bg-gray-300 text-gray-500 hover:bg-gray-300 cursor-default')}>{joined ? 'Joined' : 'Join'}</ThemedButton>
